...
namespace Repositories.Repositories.Base
{
     ...
     where TEntity : class, new()
    {
        ...
        public async Task<int> UpdateMany(List<TEntity> listEntity)
        {
            ...
            return await _sqlSugarClient.Updateable(listEntity).ExecuteCommandAsync();
        }
            ...
            Expression<Func<T, T2, T3, bool>> whereLambda = null) where T : class, new()
        {
            if (whereLambda == null)
            {
                return await _sqlSugarClient.Queryable(joinExpression).Select(selectExpression).ToListAsync();
            }
            return await _sqlSugarClient.Queryable(joinExpression).Where(whereLambda).Select(selectExpression).ToListAsync();
        }
        ...
        /// <param name="whereExpression">条件表达式</param>
        /// <param name="intPageIndex">页码数</param>
        /// <param name="intPageSize">每页条数</param>
        /// <returns></returns>

        public async Task<List<TEntity>> SqlQuery(string sql, Expression<Func<TEntity, bool>> whereExpression, int intPageIndex, int intPageSize)
        {
            return await _sqlSugarClient.SqlQueryable<TEntity>(sql).Where(whereExpression).ToPageListAsync(intPageIndex, intPageSize);
        }
